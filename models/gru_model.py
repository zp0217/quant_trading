...
def train_gru(df: pd.DataFrame, lookback: int = None, epochs: int = 50,
    ...
    if n < MIN_ROWS:
        return {"error": f"GRU: {n} rows of data (minimum {MIN_ROWS} rows required)",
    ...
    if train_end >= val_end:
        train_end = val_end - 2
    if train_end < lb + 2:
        return {"error": f"GRU: Insufficient data after split (n={n}, lb={lb})",
                "insufficient_data": True}
    ...
    if len(X_train) < 3:
        return {"error": f"GRU: Insufficient train sequences ({len(X_train)})",
                "insufficient_data": True}

    model = Sequential([
        GRU(32, return_sequences=True, input_shape=(lb, len(FEATURES))),
        Dropout(0.2),
